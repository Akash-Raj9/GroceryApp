  ...
  return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            {   open && (
                ...
                <NavLink to='/Allproducts' onClick={()=>{setOpen(false)}} >my Products</NavLink>
                {user && <NavLink to='/Allproducts' onClick={()=>{setOpen(false)}} >MyOrders</NavLink>
                }
                <NavLink to='/' onClick={()=>{
                    setOpen(false);
                ...
                {!user ? (<button onClick={()=>{setOpen(false)}} className="cursor-pointer px-6 py-2 mt-2 bg-indigo-500 hover:bg-primary transition text-white rounded-full text-sm">
                    Login
                </button>) : (
